...
// funzione per quando non c'è img icona utente
const postMetaIcon = document.querySelectorAll('.post-meta__icon');

postMetaIcon.forEach((document, i) => {
...
likesButton.forEach((document)=>{

    const buttonLike = document.querySelector('.like-button');
    const jsLikeButton = document.querySelector('.js-likes-counter');
    ...
    buttonLike.addEventListener('click', ()=> {
        ...
        if(buttonLike.className === 'like-button js-like-button like-button--liked'){
            jsLikeButton.innerHTML = defaultLike + 1;
            likedPostsIDs[i][nameIdLike] = true;

        }else {
            jsLikeButton.innerHTML = defaultLike - 1;
            likedPostsIDs[i][nameIdLike] = false;
        }
